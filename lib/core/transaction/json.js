...
function fromJSONToSignedTransaction(data) {
    var sig = data.sig, blockNumber = data.blockNumber, blockHash = data.blockHash, transactionIndex = data.transactionIndex, result = data.result;
    if (typeof sig !== "string") {
        throw Error("Unexpected type of sig");
    }
    if (blockNumber) {
        return new classes_1.SignedTransaction(fromJSONToTransaction(data), sig, blockNumber, new classes_1.H256(blockHash), transactionIndex, result);
    }
    else {
